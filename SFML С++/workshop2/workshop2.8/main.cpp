...
bool areCloseAbsolute(float, float, float);
bool areCloseRelative(float, float, float);
bool areFuzzyEqual(float, float);
...
void initNewBall(PRNG generator, vector<Ball>& balls, Ball& ball, set<Colr>& isColrs)
{
    ...
    ball.lifeTime = 0.f;
    ball.shape.setRadius(random_index(generator, 60));
    ...
    for (size_t fi = 0; fi < balls.size(); ++fi)
    {
        for (size_t si = fi + 1; si < balls.size(); ++si)
        {
            ...
            {  
                ...
                const float firstLeftImpulse = getWeight(first.shape.getRadius()) * length(first.offset);
                const float secondLeftImpulse = getWeight(second.shape.getRadius()) * length(second.offset);

                const float lastImpulse = firstLeftImpulse + secondLeftImpulse;
                
                const float firstRightImpulse = getWeight(first.shape.getRadius()) * length(newFirstSpeed);
                const float secondRightImpulse = getWeight(second.shape.getRadius()) * length(newSecondSpeed);
                
                const float newImpulse = firstRightImpulse + secondRightImpulse;

                const float firstLeftEnergy = getWeight(first.shape.getRadius()) * sqr(length(first.offset));
                const float secondLeftEnergy = getWeight(second.shape.getRadius()) * sqr(length(second.offset));
                
                const float lastEnergy = firstLeftEnergy + secondLeftEnergy;
                
                const float firstRightEnergy = getWeight(first.shape.getRadius()) * sqr(length(newFirstSpeed));
                const float secondRightEnergy = getWeight(second.shape.getRadius()) * sqr(length(newSecondSpeed));

                const float newEnergy = firstRightEnergy + secondRightEnergy;

                assert(areFuzzyEqual(lastImpulse, newImpulse));
                assert(areFuzzyEqual(lastEnergy, newEnergy));
...
void initDefaultPlayGround(PRNG generator, vector<Ball>& balls, Ball& ball, set<Colr>& isColrs)
{
    for (int i = 0; i < BALLS_COUNT; ++i)
    {
        ...
        balls.at(i).shape.setRadius(random_index(generator, 60));
        Colr colr = initColor(generator, isColrs);
